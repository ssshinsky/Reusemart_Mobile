...
class _ConsignmentHistoryPageState extends State<ConsignmentHistoryPage> {
  ...
  Future<void> _loadHistory() async {
    try {
      final history = await widget.apiClient.getConsignmentHistoryById(widget.penitipId);
      setState(() {
        _history = history;
        _isLoading = false;
      });
                                      ...
                                      Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: transaction.status == 'Selesai'
                                              ? Colors.green
                                              : transaction.status == 'Dalam Proses'
                                                  ? Colors.orange
                                                  : Colors.redAccent,
  ...
  Widget _buildInfoRow(BuildContext context, String label, String value, double screenWidth) {
    final theme = Theme.of(context);
    return Padding(
